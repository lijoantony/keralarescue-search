  ...
  render() {
   ...
   if (this.props.multiselect) {
      facitems = buckets.map(bucket => {
        ...
        return <li className={selected?"app_bold" : "app_normal"} key={key}>
          <input id={key} type="checkbox" onChange={handler}
           value={bucket.val} checked={selected}/>{" "}
          <label htmlFor={key}>{bucket.val} ({bucket.count})</label>
        </li>;
      });
    } else {
      facitems = buckets.map(bucket => {
        ...
        if (selected) {
          return <li key={key}><label className="app_underline">
